...
import com.example.food.MessegerActivity;
...
public class ChatsAdapter extends RecyclerView.Adapter<ChatsAdapter.ChatHolder> {
    ...
    private List<Store> users;
    private boolean isChat;

    String theLastMessage;
    Long theTimeMessage;
    public ChatsAdapter(Context mContext, List<Store> users, boolean isChat) {
    ...
    public void onBindViewHolder(@NonNull ChatHolder holder, int position) {
        final Store user = users.get(position);
        ...
        if(isChat){
            lastMessage(user.getTokenstore(),holder.tvItemMess,holder.tvItemTime);
        }else{
...
//        }
        ...
        holder.itemChats.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("userId", user.getTokenstore());
